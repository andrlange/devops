# Upgrade to Stack v1.1.2

## Fresh Installation

Remove any previous installer files and download v1.1.2 in one command:

```bash
rm -f installer.sh stack.tgz && \
  curl -sfL https://artifactory.cfapps.cool/api/v1/repositories/generic/download/installer-v1.1.2.sh -o installer.sh && \
  curl -sfL https://artifactory.cfapps.cool/api/v1/repositories/generic/download/stack-v1.1.2.tgz -o stack.tgz && \
  bash installer.sh
```
